...
public class ZephyrOverlayFeatureRenderer<T extends LivingEntity> extends FeatureRenderer<T, ZephyrEntityModel<T>> {
    private final EntityModel<T> model = new ZephyrEntityModel<T>(false);

    public ZephyrOverlayFeatureRenderer(FeatureRendererContext<T, ZephyrEntityModel<T>> featureRendererContext) {
        super(featureRendererContext);
    }  

    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T livingEntity, float f, float g, float h, float j, float k, float l) {
        if (!livingEntity.isInvisible()) {
            ((ZephyrEntityModel<T>)this.getContextModel()).copyStateTo(this.model);
