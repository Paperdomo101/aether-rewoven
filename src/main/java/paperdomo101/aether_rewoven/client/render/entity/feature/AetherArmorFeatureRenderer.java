...
public class AetherArmorFeatureRenderer <T extends LivingEntity, M extends BipedEntityModel<T>, A extends BipedEntityModel<T>> extends FeatureRenderer<T, M> {
    private static final Map<String, Identifier> ARMOR_TEXTURE_CACHE = Maps.newHashMap();
    private final A leggingsModel;
    private final A bodyModel;

    public AetherArmorFeatureRenderer(FeatureRendererContext<T, M> context, A leggingsModel, A bodyModel) {
        super(context);
    ...
    private void renderArmor(MatrixStack matrices, VertexConsumerProvider vertexConsumers, T livingEntity, EquipmentSlot equipmentSlot, int i, A bipedEntityModel) {
        ...
        if (itemStack.getItem() instanceof ArmorItem) {
            ...
            if (armorItem.getSlotType() == equipmentSlot) {
                ((BipedEntityModel<T>)this.getContextModel()).setAttributes(bipedEntityModel);
    ...
    private void renderArmorParts(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, ArmorItem armorItem, boolean bl, A bipedEntityModel, boolean bl2, float f, float g, float h, String string) {
    ...
    private Identifier getArmorTexture(ArmorItem armorItem, boolean bl, String string) {
        String string2 = "aether_rewoven:textures/models/armor/" + armorItem.getMaterial().getName() + "_layer_" + (bl ? 2 : 1) + (string == null ? "" : "_" + string) + ".png";
