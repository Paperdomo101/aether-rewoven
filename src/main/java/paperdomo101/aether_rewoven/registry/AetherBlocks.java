...
public class AetherBlocks {
    ...
    public static final Block ICESTONE = register("icestone", new Block(FabricBlockSettings.copyOf(Blocks.STONE).materialColor(MaterialColor.WHITE).breakByTool(FabricToolTags.PICKAXES).strength(3.0f, 4.0f)), AetherRewoven.AETHER_BLOCKS);
    ...
    public static final Block AMBROSIUM_TORCH = new AetherTorchBlock(AbstractBlock.Settings.of(Material.SUPPORTED).noCollision().breakInstantly().lightLevel((state) -> {return 9;}).sounds(BlockSoundGroup.WOOD), AetherParticles.AMBROSIUM_DUST, false);
    public static final Block AMBROSIUM_WALL_TORCH = new AetherWallTorchBlock(AbstractBlock.Settings.of(Material.SUPPORTED).noCollision().breakInstantly().lightLevel((state) -> {return 9;}).sounds(BlockSoundGroup.WOOD).dropsLike(AMBROSIUM_TORCH), AetherParticles.AMBROSIUM_DUST, false);
